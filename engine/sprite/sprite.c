...
static void drawspritey(t_vars *vars, int stripe)
{
	...
	{
		int d = y * 256 - vars->screen.screen_h * 128 + \
		vars->spr.spriteheight * 128;
		int texY = (d * vars->tex.h[4]) / vars->spr.spriteheight / 256;
		vars->color = *(unsigned int*)(vars->tex.addr[4] + (texY * \
...
void sprite(t_vars *vars, int i)
{
	...
	vars->spr.drawend_y = vars->spr.spriteheight / 2 + vars->screen.screen_h / 2;
	if (vars->spr.drawend_y >= vars->screen.screen_h)
		vars->spr.drawend_y = vars->screen.screen_h - 1;
	vars->spr.spritewidth = fabs((int)vars->screen.screen_h / vars->spr.transform_y);
	vars->spr.drawstart_x = -vars->spr.spritewidth / 2 + vars->spr.spritescreenx;
	if (vars->spr.drawstart_x < 0)
		vars->spr.drawstart_x = 0;
	vars->spr.drawend_x = vars->spr.spritewidth / 2 + vars->spr.spritescreenx;
